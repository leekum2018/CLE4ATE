...
'''
Transformer Encoder, we apply on last three layers 
...
class TransformerXHEncoder(BertEncoder):
    ...
    def forward(self, graph, hidden_states, attention_mask, gnn_layer_num, output_all_encoded_layers=False):

        all_encoder_layers = []
        for i, layer_module in enumerate(self.layer):
            
            pooled_output = hidden_states[:, 0]#两者数据储存位置一样
            graph.ndata['h'] = pooled_output  #两者数据储存位置不一样
